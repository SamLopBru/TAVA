...
def train(model: nn.Module, dataloaders: dict[str, DataLoader], criterion: nn.Module, optimizer: torch.optim.Optimizer, device: torch.device, num_epochs: int, checkpoint_every: bool = False, save_dir: str = "outputs", scheduler = None):
    ...
    for epoch in range(num_epochs):
        epoch_loss, epoch_dice, epoch_iou = epoch_train(model, train_loader, criterion, optimizer, device)
        val_loss, val_dice, val_iou = epoch_val(model, val_loader, criterion, device)
        ...
        print(f"  Train  -> Loss: {epoch_loss:.4f} | Dice: {epoch_dice:.4f} | IoU: {epoch_iou:.4f}")
